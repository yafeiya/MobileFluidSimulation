...
//
//}
VkShaderModule Renderer::MakeShaderModule(const char *filename,android_app* app) {
    ...
    if (fileSize <= 0) {
        AAsset_close(asset);
        throw std::runtime_error("Shader file is empty: " + std::string(filename));
    }

    // 读取文件内容
    bytes.resize(fileSize);
    AAsset_read(asset, bytes.data(), fileSize);
    AAsset_close(asset);
...
void Renderer::Init(android_app* app)
{
    ...
    CreateDefaultTextureResources(app);
...
void Renderer::CreateInstance()
{
    ...
    appinfo.pApplicationName = "mytest";
    appinfo.applicationVersion = VK_MAKE_VERSION(1, 0, 0); //new under this
...
void Renderer::CreateSurface(ANativeWindow* platformWindow){
    // 使用 nativeWindow 创建 Vulkan Surface
...
void Renderer::CreateDefaultTextureResources(android_app* app)
{

    int texWidth,texHeight,texChannels;
    //stbi_uc* pixels = stbi_load("D:/C_pro/mytest/app/src/main/textures/default_texture.png",&texWidth,&texHeight,&texChannels,STBI_rgb_alpha);
    stbi_uc *pixels;
    AAsset* asset = AAssetManager_open(app->activity->assetManager, "default_texture.png", AASSET_MODE_UNKNOWN);
    if (asset) {
        off_t size_t = AAsset_getLength(asset);
        char *imgData = new char[size_t];
        AAsset_read(asset, imgData, size_t);
        AAsset_close(asset);

        pixels = stbi_load_from_memory((unsigned char *) imgData, size_t, &texWidth,
                                                &texHeight, &texChannels, STBI_rgb_alpha);
    }

    if (pixels==NULL)
    {
        printf("加载错误%s",stbi_failure_reason());
    }
...
void Renderer::CreateDescriptorPool()
{
    ...
    poolsizes[0].descriptorCount = 16;   //ori：8
    poolsizes[0].type = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
    poolsizes[1].descriptorCount = 16;  //ori：8
    poolsizes[1].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    poolsizes[2].descriptorCount = 40; //ori:16
    poolsizes[2].type = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
    poolsizes[3].descriptorCount = 16;  //ori：8
    poolsizes[3].type = VK_DESCRIPTOR_TYPE_STORAGE_IMAGE;


    VkDescriptorPoolCreateInfo createinfo{};
    createinfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO;
    createinfo.maxSets = 32;  //ori:16
...
void Renderer::CreateGraphicPipeline(android_app* app)
{
    auto fluidvertshadermodule = MakeShaderModule("shaders/spv/fluidvertshader.spv",app);
    auto fluidfragshadermodule = MakeShaderModule("shaders/spv/fluidfragshader.spv",app);
    ...
    auto boxvertshadermodule = MakeShaderModule("shaders/spv/boxvertshader.spv",app);
    auto boxfragshadermodule = MakeShaderModule("shaders/spv/boxfragshader.spv",app);
...
void Renderer::CreateComputePipeline(android_app* app)
{
    {
        //NGBR PIPELINES
        auto computeshadermodule_calcellhash = MakeShaderModule("shaders/spv/compshader_calcellhash.spv",app);
        auto computeshadermodule_radixsort1 = MakeShaderModule("shaders/spv/compshader_radixsort1.spv",app);
        auto computeshadermodule_radixsort2 = MakeShaderModule("shaders/spv/compshader_radixsort2.spv",app);
        auto computeshadermodule_radixsort3 = MakeShaderModule("shaders/spv/compshader_radixsort3.spv",app);
        auto computeshadermodule_fixcellbuffer = MakeShaderModule("shaders/spv/compshader_fixcellbuffer.spv",app);
        auto computeshadermodule_getngbrs = MakeShaderModule("shaders/spv/compshader_getngbrs.spv",app);
...

    {
        ...
        auto computershadermodule_euler = MakeShaderModule("shaders/spv/compshader_euler.spv",app);
        auto computershadermodule_lambda = MakeShaderModule("shaders/spv/compshader_lambda.spv",app);
        auto computershadermodule_deltaposition = MakeShaderModule("shaders/spv/compshader_deltaposition.spv",app);
        auto computershadermodule_positionupd = MakeShaderModule("shaders/spv/compshader_positionupd.spv",app);
        auto computershadermodule_velocityupd = MakeShaderModule("shaders/spv/compshader_velocityupd.spv",app);
        auto computershadermodule_velocitycache = MakeShaderModule("shaders/spv/compshader_velocitycache.spv",app);
        auto computershadermodule_viscositycorr = MakeShaderModule("shaders/spv/compshader_viscositycorr.spv",app);
        auto computershadermodule_vorticitycorr = MakeShaderModule("shaders/spv/compshader_vorticitycorr.spv",app);
    ...
    }
    {
        ...
        auto computershadermodule_postprocessing = MakeShaderModule("shaders/spv/compshader_postprocessing.spv",app);
        auto computershadermodule_filtering = MakeShaderModule("shaders/spv/compshader_filtering.spv",app);
...
//    exts.push_back(VK_KHR_ANDROID_SURFACE_EXTENSION_NAME); // 添加 Android Surface 扩展
//    exts.push_back(VK_EXT_EXTENDED_DYNAMIC_STATE_3_EXTENSION_NAME);
//    //exts.push_back(VK_EXT_SHADER_OBJECT_EXTENSION_NAME);//"VK_EXT_shader_object"
//    exts.push_back("VK_EXT_shader_object");
